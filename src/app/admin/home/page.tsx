...
export default function AdminHomePage() {
  ...
  const [originalHeroContent, setOriginalHeroContent] = useState("");
  ...
  // Check for unsaved changes in hero content
  const hasUnsavedChanges = JSON.stringify(heroContent) !== originalHeroContent;
  ...
  const getImageUrl = useCallback((artwork: FeaturedArtwork) => {
    const image = Array.isArray(artwork.images) ? artwork.images[0] : artwork.images;
    return `${pbUrl}/api/files/${artwork.collectionId}/${artwork.id}/${image}?thumb=200x200`;
  }, [pbUrl]);
        ...
        try {
          ...
          settings.items.forEach((item) => {
            if (item.section === "hero") {
              ...
              setOriginalHeroContent(JSON.stringify(content));
            }
          });
        } catch {
  ...
  const handleSaveHero = async () => {
    ...
    try {
      ...
      setOriginalHeroContent(JSON.stringify(heroContent));
      ...
      {hasUnsavedChanges && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-center gap-3">
          ...
          <p className="text-yellow-800 text-sm">
            <strong>Modifications non enregistrées</strong> - Cliquez sur &quot;Enregistrer&quot; pour sauvegarder vos changements.
          </p>
        </div>
      )}
